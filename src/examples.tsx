...
export const SimpleCalendar: React.FC<{
  ...
  return (
    <div className="calendar" style={{ fontFamily: 'Arial, sans-serif', width: '280px' }}>
      {/* Header */}
      <div 
...
export const RangePicker: React.FC<{
  ...
  return (
    <div className="range-picker" style={{ fontFamily: 'Arial, sans-serif', width: '280px' }}>
      {/* Header */}
      <div 
        className="calendar-header" 
        style={{ 
      ...
      <div style={{ padding: '8px', fontSize: '12px', color: '#666', textAlign: 'center' }}>
        {datePicker.selectedRange.start && datePicker.selectedRange.end
          ...
          : datePicker.selectedRange.start
            ? `${datePicker.formatDate(datePicker.selectedRange.start, 'MMM d, yyyy')} - Select end date`
            : 'Select start date'
        }
      </div>
          ...
          week.days.map((day, dayIndex) => (
            <button
              ...
              style={{
                ...
                fontSize: '14px',
                cursor: day.isDisabled ? 'not-allowed' : 'pointer',
                backgroundColor: day.isRangeStart || day.isRangeEnd
                  ? '#007bff'
                  : day.isInRange
                    ? '#e6f3ff'
                    : day.isToday 
                      ? '#f0f0f0' 
                      : 'transparent',
                color: day.isRangeStart || day.isRangeEnd
                  ? 'white'
                  : day.isInRange
                    ? '#007bff'
                    : day.isCurrentMonth 
...
export const MultiSelectCalendar: React.FC<{
  ...
  const handleDateClick = (date: Date) => {
    if (maxDates && datePicker.selectedDates.length >= maxDates && !datePicker.isDateSelected(date)) {
  ...
  return (
    <div className="multi-select-calendar" style={{ fontFamily: 'Arial, sans-serif', width: '280px' }}>
      {/* Header */}
      <div 
        className="calendar-header" 
        style={{ 
      ...
      {/* Selected count */}
      <div style={{ padding: '8px', fontSize: '12px', color: '#666', textAlign: 'center' }}>
        {datePicker.selectedDates.length} date{datePicker.selectedDates.length !== 1 ? 's' : ''} selected
      ...
      >
        {datePicker.calendar.weeks.map((week, weekIndex) =>
          week.days.map((day, dayIndex) => {
            const isLimitReached = !!(maxDates && datePicker.selectedDates.length >= maxDates && !day.isSelected);
            return (
              <button
                key={`${weekIndex}-${dayIndex}`}
                onClick={() => handleDateClick(day.date)}
                ...
                style={{
                  ...
                  borderRadius: '4px',
                  fontSize: '14px',
                  cursor: (day.isDisabled || isLimitReached) ? 'not-allowed' : 'pointer',
                  backgroundColor: day.isSelected 
                    ? '#007bff' 
                    : day.isToday 
                      ? '#f0f0f0' 
